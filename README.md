# Agent News Archive

这个仓库用于沉淀每日 Agent 方向简报，便于长期追踪、每周回顾和主题检索。

目录结构：

- `daily/`：每日技术研究摘要与最新进展，按日期单独成文
- `weekly/`：每周深度解读，聚焦趋势判断、行业思考、职业启发与真实工作场景
- `learning_weekly/`：每周学习陪跑周报，聚焦系统学习、必读材料、工作场景和动手实验
- `reports/`：专题研究报告、深度分析与可交付文档归档
- `INDEX.md`：日报、周报与学习周报索引，按时间倒序汇总
- `TOPICS.md`：主题索引，按产品、技术方向和研究议题整理

约定：

- 每篇日报文件名使用 `YYYY-MM-DD.md`
- 每篇日报标题使用 `# Agent 日报 | YYYY-MM-DD`
- 每篇周报文件名使用 `YYYY-Www.md`
- 每篇周报标题使用 `# Agent 周报 | YYYY-Www`
- 每篇学习周报文件名使用 `YYYY-Www.md`
- 每篇学习周报标题使用 `# AI 学习周报 | YYYY-Www`
- 每篇日报建议在开头增加 `标签` 小节，使用 `#HarnessEngineering` 这类固定标签

默认重点关注的信息源：

- The Rundown AI: https://www.therundown.ai/
- TLDR AI: https://tldr.tech/ai
- The Neuron: https://www.theneurondaily.com/
- One Useful Thing: https://www.oneusefulthing.org/
- Ben's Bites: https://www.bensbites.com/
- Latent Space: https://www.latent.space/
- Interconnects: https://www.interconnects.ai/
- AI Valley: https://www.theaivalley.com/
